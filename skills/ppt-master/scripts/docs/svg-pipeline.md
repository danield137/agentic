...
import/export. It uses only a gitignored `projects/_smoke_*` workspace; keep it
as a manual smoke procedure, not a test file or example deck.
...
- Transition/object sound remains off by default. Use only compatible user-provided audio that already exists in the project. `transition.sound` references a project-relative `.wav`; object-animation `sound` accepts the existing `.m4a`/`.mp3`/`.wav` path contract. With no supplied cue, do not create `<project>/sounds/`
