...
| **B** | Native PowerPoint shapes | Recurring compact semantic cues that simple geometry can express |
...
AI illustrated icons are not a base option, add-on, field, or key — like decorative lettering they are a downstream image carrier §h and [`strategist-image.md`](./strategist-image.md) may choose, with slices under `images/` (never `icons/`, `icons.inventory`, or `<use data-icon>`); they may coexist with project-local icons. Real brand marks require an exact supplied, imported, or deliberately authored asset; never substitute a similar mark.

**Mandatory — core icon resources**:

1. At confirmation choose native shapes, custom project icons, Emoji, or none.
2. Use only SVG files that already exist under `<project_path>/icons/`; record their complete case-sensitive `library/name` identifiers in §VI and `spec_lock.md`.
3. For missing icons, use a native shape, author a simple project-local SVG with user-approved provenance, or omit the icon. Never run `icon_sync.py` or download a library automatically. See [`../templates/icons/README.md`](../templates/icons/README.md).

🚧 **GATE — missing icon = re-plan now**: never carry a missing icon forward or reference a file that does not exist.
