...
### §6.1 Main agent → reviewer (batched dispatch)

The main agent partitions N pages into `ceil(N/K)` batches of ≤ K pages
(default **K = 5**, configurable per run). When Copilot CLI task agents are
available, launch one independent general-purpose reviewer per batch in one
parallel tool call; otherwise process batches sequentially. Each prompt is
self-contained — absolute paths for inputs 1–5 plus the full batch records; no
prior context assumed. Reviewers need file read/edit and command execution, but
do not run Playwright because the main agent pre-renders.
...
### §6.2 Reviewer → main agent

Each reviewer returns one JSON path per processed page and a ≤150-word summary
of the batch in its task result. A mid-batch abort still reports aborted pages
as `needs_human` or `render_failed`.

### §6.3 Main-agent aggregation

Aggregate the task results into the page × status × hard_hits × soft_hits ×
fixes_applied × needs_human_reason table, add one ≤150-word plumbing verdict,
and include the `brand_review.json` path if any §1.1 aggregation occurred.
