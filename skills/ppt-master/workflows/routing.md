...
|---|---|---|---|---|---|
| Generate PPTX | Create, reconstruct, or visually regenerate a presentation/video from sources or a topic; templates optional | Beautify: [`beautify-pptx`](./profiles/beautify-pptx.md), Default or Quick; ordinary [`generate-pptx`](./generate-pptx.md) / [`quick-generate`](./profiles/quick-generate.md) | Facts exist or research can gather them | Author SVG pages and export a new PPTX | Default: spec/lock/SVG/PPTX; Quick: optional source/resource artifacts, no spec/lock, SVG/PPTX; either may derive narrated PPTX/MP4 |
...
|---|---|
| Raster files represent page frames to recreate | Use ordinary [`quick-generate`](./profiles/quick-generate.md); preserve visible wording and geometry, but do not claim hidden-layer recovery or pixel-perfect reconstruction |
...
| User requests preview, selection, or annotation application | Default pipeline plus [`live-preview`](./stages/live-preview.md) at its defined stage; explicit Quick + preview intent falls back to Default rather than dropping preview |
...
**Hard rule — fidelity profile, not a fourth route**: Beautify changes source/page invariants inside Generate and uses Quick only on explicit Quick intent. It does not define a separate lifecycle.
...
|---|---|
| Images containing page frames + explicit final-deck reconstruction intent | Generate PPTX with ordinary Quick; treat frames as visible references and never infer hidden structure or semantics from pixels |
