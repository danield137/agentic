...
| Visual-reference reconstruction asset | Preserve identity graphics through an exact supplied vector, deterministic redraw, or sufficient source asset; keep data graphics native-and-verified or exact; never invent hidden layers or semantics |
| Project-local SVG icon | Follow the [core icon contract](../../templates/icons/README.md); use supplied, imported, or deliberately authored project-local SVGs only |
