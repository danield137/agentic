...
Only after visual transition, lifecycle, effect, order, and timing are complete; sound is post-processing state never written to or recovered from the Design Spec or lock. No resolved cue → omit every `sound` field and create no `sounds/`. Use only compatible user-provided audio that already exists in the project, referenced by its validated project-relative path. Never run `sound_sync.py` or download a sound library. Gain, limiting, and video mixing belong to [`generate-audio`](./generate-audio.md), never this sidecar.
