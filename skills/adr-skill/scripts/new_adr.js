#!/usr/bin/env node
/**
 * Create a new ADR markdown file using repo conventions and the MADR bare template.
 *
 * Design goals:
 * - Safe defaults (auto-detect ADR directory + filename strategy)
 * - No external deps
...
function parseArgs(argv) {
    ...
    if (a === '--repo-root') out.repoRoot = next();
      ...
      process.stdout.write(
        [
          ...
          '  --template bare           Template (default: bare; only supported value)',
  ...
  if (out.template !== 'bare')
    die(`Invalid --template: ${out.template}. Only 'bare' is supported.`);
...
function renderTemplate(raw, vars) {
  let out = raw;
  out = replaceFrontMatterField(out, 'status', vars.status);
  out = replaceFrontMatterField(out, 'date', vars.date);
  out = replaceFrontMatterField(out, 'decision-makers', vars.decisionMakers);
  out = replaceFrontMatterField(out, 'consulted', vars.consulted);
  out = replaceFrontMatterField(out, 'informed', vars.informed);
  out = out.replace(
    /^#\s+<!-- short title[^>]*-->\s*$/m,
    `# ${vars.title}`,
  );
...
function main() {
  ...
  const rendered = renderTemplate(raw, {
    ...
    date: today,
    decisionMakers: String(args.decisionMakers || '').trim(),
    consulted: String(args.consulted || '').trim(),
    informed: String(args.informed || '').trim(),
  });
